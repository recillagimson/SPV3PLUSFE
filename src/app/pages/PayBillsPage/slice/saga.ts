...
import { selectBillerCode, selectFormData } from './selectors';
...
function* validatePaybills() {
  ...
  const requestURL = `${process.env.REACT_APP_API_URL}/pay/bills/validate/account/${billerCode}/${formData.account_number}`;
  ...
  try {
    ...
    if (apirequest) {
      ...
      console.log('validate pay bills', decryptData);
